...
typedef struct stmTyp_t {
	...
	uint16_t outPeriod, realPeriod, finalPeriod, finalVol, tonTyp, wantPeriod,
	    portaSpeed;
	...
	uint8_t glissFunk, vibPos, tremPos, vibSpeed, vibDepth, tremSpeed,
	    tremDepth;
	uint8_t pattPos, loopCnt, volSlideSpeed, fVolSlideUpSpeed,
	    fVolSlideDownSpeed;
	...
	uint8_t volKolVol, tonNr, envPPos, eVibPos, envVPos, realVol, oldVol,
	    outVol;
	...
	uint8_t
	    ntxmVolFadeLast; ///< Last volume set to the audio channel before fading began
	uint16_t ntxmVolFadeTicks; ///< The number of total ticks for voluem fading
	uint16_t
	    ntxmVolFadeTicksLeft; ///< The number of remaining ticks for voluem fading
} stmTyp;
