...
void Sample::play(u8 note, u8 volume_, u8 channel, u8 offs)
{
	...
	*/
	...
	if (loop == NO_LOOP)
	{
		ntxm_sound_channel_set_source(channel, sound_data + offs_samps, 0, size - offs_samps);
	}
	else if( loop == FORWARD_LOOP || (loop == PING_PONG_LOOP && !pingpong_data) )
	{
		u32 loop_offs_samps = ntxm_clamp(offs_samps, 0, loop_start);
		ntxm_sound_channel_set_source(channel, sound_data + loop_offs_samps, loop_start - loop_offs_samps, loop_length);
	}
	else if( loop == PING_PONG_LOOP )
	{
		u32 loop_offs_samps = ntxm_clamp(offs_samps, 0, loop_start);
		ntxm_sound_channel_set_source(channel, pingpong_data + loop_offs_samps, loop_start - loop_offs_samps, loop_length << 1);
	}
