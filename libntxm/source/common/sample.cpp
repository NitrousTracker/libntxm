...
#define LOOKUP_FREQ(note, finetune)                                            \
...
uint32_t ntxmGetFrequencyValue(uint16_t period, bool linear)
{
	...
	if (linear) {
		...
		return (uint32_t)(((int64_t)logTab[remainder] * 2140928) >> 24) >>
		       octShift;
	} else {
...
Sample::Sample(void *_sound_data, u32 _n_samples, u16 _sampling_frequency,
               bool _is_16_bit, u8 _loop, u8 _volume)
    : pingpong_data(0), n_samples(_n_samples), is_16_bit(_is_16_bit),
      loop(_loop), loop_start(0), loop_length(0), volume(_volume), panning(128)
{
      ...
      panning(128)
{
	...
	setLoopStartAndLength(wav.getLoopStart(),
	                      wav.getLoopEnd() - wav.getLoopStart() + 1);
...
void Sample::play(u8 channel, u8 panning, u8 volume, u8 offs)
{
	...
	u32 loop_bit;
	if (((loop == FORWARD_LOOP) || (loop == PING_PONG_LOOP)) &&
	    (loop_length > 0))
	...
	u32 offs_samps = FT_OFFSET_SCALAR * offs *
	                 (sound_format == NTXMSOUND_FORMAT_8BIT ? 1 : 2);
	...
	if (loop == NO_LOOP) {
		ntxm_sound_channel_set_source(
		    channel, (uint8_t *)sound_data + offs_samps, 0, size - offs_samps);
	} else if (loop == FORWARD_LOOP ||
	           (loop == PING_PONG_LOOP && !pingpong_data)) {
		u32 loop_offs_samps = ntxm_clamp(offs_samps, 0, loop_start);
		ntxm_sound_channel_set_source(
		    channel, (uint8_t *)sound_data + loop_offs_samps,
		    loop_start - loop_offs_samps, loop_length);
	} else if (loop == PING_PONG_LOOP) {
		u32 loop_offs_samps = ntxm_clamp(offs_samps, 0, loop_start);
		ntxm_sound_channel_set_source(
		    channel, (uint8_t *)pingpong_data + loop_offs_samps,
		    loop_start - loop_offs_samps, loop_length << 1);
	}
...
void Sample::setRelNote(s8 _rel_note) { rel_note = _rel_note; }

void Sample::setFinetune(s8 _finetune) { finetune = _finetune; }

#endif

u8 Sample::getRelNote(void) { return rel_note; }

s8 Sample::getFinetune(void) { return finetune; }

u32 Sample::getSize(void) { return size; }

u32 Sample::getNSamples(void) { return n_samples; }

void *Sample::getData(void) { return sound_data; }

u8 Sample::getLoop(void) { return loop; }

#ifndef ARM7

bool Sample::setLoop(
    u8 loop_) // Set loop type. Can fail due to memory constraints
{
	if (loop_ == loop)
		return true;
...
bool Sample::is16bit(void) { return is_16_bit; }
...
{
	u32 min_loop_length =
	    (4 >> (is_16_bit ? 1 : 0)) >> (loop == PING_PONG_LOOP ? 1 : 0);

	// Clamping
	if (_loop_start >= (n_samples - min_loop_length))
		_loop_start = n_samples - min_loop_length;
	if (_loop_length >= (n_samples - _loop_start))
		_loop_length = n_samples - _loop_start;
...
void Sample::setVolume(u8 vol) { volume = vol; }

u8 Sample::getVolume(void) { return volume; }

void Sample::setPanning(u8 pan) { panning = pan; }

u8 Sample::getPanning(void) { return panning; }
...
const char *Sample::getName(void) { return name; }
...
void Sample::delPart(u32 startsample, u32 endsample)
{
	...
	if (endsample < n_samples - 1) {
		memmove((u8 *)sound_data + startsample * bps,
		        (u8 *)sound_data + (endsample + 1) * bps,
		        ((n_samples - 1) - endsample) * bps);
	}
...
{
	if (*(u16 *)elem1 < *(u16 *)elem2)
		return -1;
	else if (*(u16 *)elem1 == *(u16 *)elem2)
		return 0;
	else
		return 1;
}
...
void Sample::fade(u32 startsample, u32 endsample, bool in)
{
	...
	if (is_16_bit == true) {
		...
		if (in == true) {
			for (s32 i = 0; i < length; ++i) {
				sounddata[offset + i] =
				    (((i * 1024) / length) * sounddata[offset + i]) / 1024;
			}
		} else {
			for (s32 i = 0; i < length; ++i) {
				sounddata[offset + i] =
				    ((((length - i) * 1024) / length) * sounddata[offset + i]) /
				    1024;
			}
		}
	} else {
		...
		if (in == true) {
			...
			for (s32 i = 0; i < length; ++i) {
				sounddata[offset + i] =
				    (length - i) * sounddata[offset + i] / length;
			}
		}
	}
...
bool Sample::setupPingPongLoop(void)
{
	...
	memcpy((u8 *)pingpong_data + pos + loop_length, (u8 *)sound_data + pos,
	       size - pos);
